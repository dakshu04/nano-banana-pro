...
export default function FaceSwap() {
  ...
  const handleTargetUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
        ...
        setGeneratedImage(null); // Reset result if new target upload
    };
    reader.readAsDataURL(file);
  };
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-6 min-h-0">
        ...
        <div className="flex flex-col h-full bg-zinc-50 rounded-2xl border border-zinc-200 overflow-hidden relative">
            ...
            <div className="flex-1 relative min-h-0 group">
                {!sourceImage ? (
                    <label className="absolute inset-0 cursor-pointer flex flex-col items-center justify-center p-6 text-center hover:bg-zinc-100 transition-colors">
                        ...
                        <div className="w-12 h-12 bg-white rounded-xl shadow-sm border border-zinc-200 flex items-center justify-center text-zinc-400 mb-4 group-hover:scale-110 transition-transform">
                            <UploadIcon />
                        </div>
                    ...
                    <div className="absolute inset-0 p-8 flex items-center justify-center">
                         <img src={sourceImage} alt="Source" className="max-w-full max-h-full object-contain drop-shadow-sm rounded-lg" />
                         {/* Hover to change */}
                         <label className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center cursor-pointer">
                             <input type="file" accept="image/*" onChange={handleSourceUpload} className="hidden" />
                             <span className="text-white text-xs font-bold border border-white/50 px-3 py-1 rounded-full">Change Photo</span>
                         </label>
                    </div>
                )}
            </div>
            ...
            <div className="h-16 border-t border-zinc-200 bg-white p-4 flex items-center justify-between shrink-0">
                 ...
                 {sourceImage ? (
                     <span className="text-xs font-bold text-emerald-600 flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500"/> Ready</span>
                 ) : (
                    <span className="text-xs text-zinc-300">Waiting...</span>
                 )}
            </div>
        </div>
        ...
        <div className="flex flex-col h-full bg-white rounded-2xl border border-zinc-200 overflow-hidden relative shadow-sm">
            ...
            <div className="flex-1 relative min-h-0 group">
                {generatedImage ? (
                     // SHOW RESULT
                     <div className="absolute inset-0 p-6 flex items-center justify-center bg-[url('https://grainy-gradients.vercel.app/noise.svg')]">
                        <img src={generatedImage} alt="Result" className="max-w-full max-h-full object-contain shadow-2xl rounded-lg" />
                     </div>
                ) : targetImage ? (
                    // SHOW UPLOADED TARGET
                    <div className="absolute inset-0 p-8 flex items-center justify-center">
                         <img src={targetImage} alt="Target" className="max-w-full max-h-full object-contain drop-shadow-sm opacity-80" />
                         <label className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center cursor-pointer">
                             <input type="file" accept="image/*" onChange={handleTargetUpload} className="hidden" />
                             <span className="text-white text-xs font-bold border border-white/50 px-3 py-1 rounded-full">Change Target</span>
                         </label>
                    </div>
                ) : (
                    // SHOW UPLOAD STATE
                    <label className="absolute inset-0 cursor-pointer flex flex-col items-center justify-center p-6 text-center hover:bg-zinc-50 transition-colors">
                        <input type="file" accept="image/*" onChange={handleTargetUpload} className="hidden" />
                        <div className="w-12 h-12 bg-zinc-50 rounded-xl border border-dashed border-zinc-300 flex items-center justify-center text-zinc-400 mb-4 group-hover:scale-110 transition-transform">
                            <UploadIcon />
                        </div>
            ...
            <div className="h-20 border-t border-zinc-100 bg-zinc-50/50 p-4 flex items-center justify-center shrink-0 z-20 gap-3">
                
                {generatedImage ? (
                    <>
                        <button 
                            onClick={resetTarget}
                            className="h-12 w-12 flex items-center justify-center rounded-xl border border-zinc-200 text-zinc-500 hover:bg-white hover:text-zinc-900 transition-all"
                            title="Reset"
                        >
                            <RefreshIcon />
                        </button>
                        <button
                            ...
                            className="flex-1 h-12 rounded-xl bg-emerald-500 text-white font-bold text-sm hover:bg-emerald-600 transition-all shadow-lg shadow-emerald-100 flex items-center justify-center gap-2"
                        >
